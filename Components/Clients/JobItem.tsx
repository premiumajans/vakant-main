...
function JobItem({
  ...
  console.log(shared_time?.split("-")[2]);
  const { i18n, t } = useTranslation("common");
  const month = shared_time?.split("-")[1];
  const date = `${+shared_time?.split("-")[2]} ${t(month)}`;

  return (
    <Link
      href={href}
      data-aos="fade-up"
      className="job-item bg-white position-relative flex-column  flex-md-row d-flex align-items-start  align-items-md-center justify-content-between bg-white"
    >
